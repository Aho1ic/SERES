...
# -*- coding: utf-8 -*-
"""
打电话姿势检测 - 图片批量推理脚本（同步版本）
YOLO识别完一张图片后，立即等待大模型二次验证完成，再处理下一张图片
python phonecall_img_test.py --input /home/algorithm/chongqing/赛力斯测试视频合集 --output /home/algorithm/chongqing/aibox/0316综合测试/phonecall2/ --model /home/algorithm/chongqing/weights/phonecall.pt --mode video --frame-interval 10
python phonecall_img_test.py --input /path/to/images --output /path/to/output --model /path/to/model.pt
"""
...
def is_making_phone_call(keypoints):
    ...
    python phonecall_img_test.py --input /home/algorithm/chongqing/aibox/jpg/ --output /home/algorithm/chongqing/aibox/phonecall/ --model /home/algorithm/chongqing/weights/phonecall.pt
...
def analyze_image_with_ai_and_resnet(image_path, bbox, temp_dir, resnet_classifier):
    ...
    resnet_result: True表示ResNet分类为打电话，False表示非打电话
    ai_result: True表示AI检测到打电话姿势，False表示未检测到
    """
    crop_path_ai = None
    crop_path_resnet = None
    ...
    try:
        ...
        try:
            ...
            resnet_result = (predicted_class == 1)  # 1表示打电话类别
            logger.info(f"ResNet分类结果: {'打电话' if resnet_result else '非打电话'} (置信度: {confidence:.3f})")
        except Exception as e:
            logger.error(f"ResNet分类失败: {e}")
            resnet_result = False
        ...
        # 删除临时裁切图片
        for crop_path in [crop_path_ai, crop_path_resnet]:
...
def process_frame(frame, frame_info, model, resnet_classifier, output_dir, temp_dir):
    ...
    只保存通过ResNet分类为phonecall的jpeg图片
    ...
    try:
        ...
        for idx, detection in enumerate(phone_call_detections):
            logger.info(f"正在验证检测框 {idx+1}/{len(phone_call_detections)}...")
        ...
        for detection in detections:
            ...
            if result_type == "main":
                ...
                if detection.get('ai_confirmed', False):
                    ...
                    label = "phoneCall(AI+ResNet)"
                else:
                    color = (0, 0, 255)  # 红色：仅ResNet确认
                    label = "phoneCall(ResNet)"
            else:
                # ai_only目录：橙色表示仅AI确认
                color = (0, 165, 255)  # 橙色：仅AI确认
                label = "phoneCall(AI_only)"
...
def main():
    """主函数"""
    try:
        parser = argparse.ArgumentParser(description='打电话姿势检测 - 图片/视频批量推理')
        ...
        # 创建临时目录（用于AI验证时的裁切图片）
        temp_dir = output_dir.parent / 'temp_phonecall'
        ...
        logger.info(f"主目录: 通过ResNet分类为phonecall的jpeg图片")
        logger.info(f"  - 绿色框: ResNet + AI都确认为打电话")
        logger.info(f"  - 红色框: 仅ResNet确认为打电话")
        logger.info(f"ai_only目录: 仅AI验证通过但ResNet未通过的图片")
        logger.info(f"  - 橙色框: 仅AI确认为打电话")
